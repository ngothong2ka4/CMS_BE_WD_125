            ...
            <form action="{{ route('product_management.update',$product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card mb-2">
                    ...
                    <div class="card-body">
                        <div>
                            ...
                            <input type="text" class="form-control" id="basiInput" name="name" value="{{$product->name}}">
                            @error('name')
                        ...
                        <div>
                            ...
                            <select name="id_materials" class="form-select mb-3" aria-label="Default select example">
                                ...
                                <option {{ ($material->id === $product->id_materials) ? 'selected' : ''}} value="{{$material->id}}">{{$material->name}}</option>
                                @endforeach
                            </select>
                            @error('id_materials')
                                <div class="invalid-feedback">
                        ...
                        <div>
                            ...
                            <select name="id_stones" class="form-select mb-3" aria-label="Default select example">
                                ...
                                <option  {{ ($stone->id === $product->id_stones) ? 'selected' : ''}} value="{{$stone->id}}">{{$stone->name}}</option>
                                @endforeach
                            </select>
                            @error('id_stones')
                                <div class="invalid-feedback">
                ...
                <div class="card mt-2">
                    ...
                    <div class="form-group me-1">
                        ...
                        <select required name="id_attribute_color[]" class="form-select mb-3" id="color-${variantCounter}" aria-label="Default select example">
                            ...
                            @foreach($colors as $color)
                                <option {{ ($color->id === $var->id_attribute_color) ? 'selected' : ''}} value="{{$color->id}}">{{$color->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group me-1">
                        ...
                        <select required name="id_attribute_size[]" class="form-select mb-3" id="size-${variantCounter}" aria-label="Default select example">
                           ...
                           @foreach($sizes as $size)
                                <option  {{ ($size->id === $var->id_attribute_size) ? 'selected' : ''}} value="{{$size->id}}">{{$size->name}}</option>
                            @endforeach
                    ...
                    <div class="me-1">
                        ...
                        <input value="{{$var->import_price}}" name="import_price[]" type="number" required min=0 class="form-control" id="price_in-${variantCounter}" placeholder="Nhập giá nhập">
                    </div>
                    <div class="me-1">
                        <label for="price_out-${variantCounter}" class="form-label">Giá niêm yết</label>
                        <input value="{{$var->list_price}}" name="list_price[]" type="number" required min=0 class="form-control" id="price_out-${variantCounter}" placeholder="Nhập giá niêm yết">
                    </div>
                    <div class="me-1">
                        <label for="sale_price-${variantCounter}" class="form-label">Giá bán</label>
                        <input value="{{$var->selling_price}}" name="selling_price[]" type="text" class="form-control" id="sale_price-${variantCounter}" placeholder="Nhập giá bán">
                    </div>
                    <div class="me-1">
